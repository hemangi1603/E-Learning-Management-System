 ...
 if(isset($_REQUEST['requpdate'])){
  ...
  if(($_REQUEST['course_id'] == "") || ($_REQUEST['course_name'] == "") || ($_REQUEST['course_desc'] == "") || ($_REQUEST['course_author'] == "") || ($_REQUEST['course_duration'] == "") || ($_REQUEST['course_price'] == "") || ($_REQUEST['course_original_price'] == "")){
   ...
   $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2" role="alert"> Fill All Fileds </div>';
    ...
    $sql = "UPDATE course SET course_name = ?, course_desc = ?, course_author = ?, course_duration = ?, course_price = ?, course_original_price = ?, course_img = ? WHERE course_id = ?";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
      $stmt->bind_param("sssssssi", $cname, $cdesc, $cauthor, $cduration, $cprice, $coriginalprice, $cimg, $cid);
      if($stmt->execute()){
      ...
      $msg = '<div class="alert alert-danger col-sm-6 ml-5 mt-2" role="alert"> Unable to Prepare Statement </div>';
    }
  }
}
 ?>
